...
(async function () {
  ...
  function renderStudents() {
    ...
    if (!students.length) {
      tableBody.innerHTML = '<tr><td colspan="5" class="text-muted">No student accounts registered yet.</td></tr>';
      return;
    }

    tableBody.innerHTML = students.slice().sort(function (a, b) { return a.name.localeCompare(b.name); }).map(function (s) {
      return '<tr>' +
        '<td style="font-weight:600;">' + BYD.escapeHtml(s.name) + '</td>' +
        '<td class="font-mono text-sm">' + BYD.escapeHtml(s.email) + '</td>' +
        '<td><span class="badge badge-role">' + BYD.escapeHtml(s.group || 'Unassigned') + '</span></td>' +
        '<td>' + (s.active ? '<span class="badge badge-present">Active</span>' : '<span class="badge badge-absent">Inactive</span>') + '</td>' +
        '<td class="table-actions">' +
        '<button class="btn btn-ghost btn-sm" data-reset-pw="' + BYD.escapeHtml(s.email) + '">Reset password</button>' +
        '<button class="btn btn-danger btn-sm" data-remove-student="' + BYD.escapeHtml(s.email) + '">Remove</button>' +
        '</td></tr>';
    }).join('');

    tableBody.querySelectorAll('[data-remove-student]').forEach(function (btn) {
      btn.addEventListener('click', function () {
        const studentEmail = btn.dataset.removeStudent;
        confirmAction('Are you sure you want to permanently remove student account ' + studentEmail + '? This action cannot be undone.', 'Remove Student', async function () {
          await BYD.call('removeStudent', { email: studentEmail });
          BYD.toast('Student account removed.', 'success');
    ...
    tableBody.querySelectorAll('[data-reset-pw]').forEach(function (btn) {
      btn.addEventListener('click', async function () {
        try {
          const data = await BYD.call('resetUserPassword', { email: btn.dataset.resetPw });
          BYD.openModal(
